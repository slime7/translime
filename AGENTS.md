...
2. Use the shared scaffold in `.agent/plugin-scaffold/`.
...
6. If the user asks for native bindings, overlay windows, or a more complex build flow, consult `.agent/plugin-scaffold/plugin-variants.md` and borrow selectively from recent packages such as `translime-plugin-steam-save-backup` or `translime-plugin-hdr-capture`.
