...
test.describe('日志查看与诊断检索 (LogViewer E2E)', () => {
  test('展示日志日期列表与日志记录，支持级别筛选与日志展示', async ({ electronContext }) => {
    ...
    if (await dateOption.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      expect(mockRecordsText).toMatch(/(?:App initialization complete|Plugin mock-test|Network request failed)/);
    }
  });
});
