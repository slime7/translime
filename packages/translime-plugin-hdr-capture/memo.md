...
### 3.5 核心流程 (Core Workflow)

*   **截图全流程**:
    1.  **触发**: 用户快捷键/按钮 -> 主进程 `start-capture`。
    2.  **捕获**: 调用 `native.capture_display` 并行获取所有屏幕图像。
    3.  **预览**: 打开全屏 Overlay (`src/ui/window/overlay.html`)，传入预览图像。
    4.  **交互**: 用户在 Overlay 上进行选区、标注、移动或窗口自动探测。
    5.  **确认**: 用户点击 Save/Copy -> 发送 `saveCapture` / `copyCapture` 到主进程。
    6.  **后处理**: `src/capture/index.js` 执行核心处理：
        *   `cropAndSaveScaledFromBuffer`: 计算多屏选区重叠，物理裁剪、缩放对齐、拼接。
        *   **HDR**: 若开启，同时处理 SDR ToneMapping 版本与原始 HDR (EXR) 版本。

### 3.6 构建与调试 (Build & Debug)
