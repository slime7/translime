...
export const cropAndGetPngFromBuffer = async (sessionData, rect) => {
  ...
  if (rect.annotationSteps && rect.annotationSteps.length > 0) {
    logger.info(`正在按顺序应用标注步骤, 数量: ${rect.annotationSteps.length}`);
    applyAnnotationSteps(finalBuffer, targetWidth, targetHeight, rect.annotationSteps, targetScale);
  } else {
    // 兼容旧数据结构
...
export const cropAndSaveScaledFromBuffer = async (sessionData, rect, options = {}) => {
  ...
  if (rect.annotationSteps && rect.annotationSteps.length > 0) {
    logger.info(`正在按顺序应用标注步骤 (save), 数量: ${rect.annotationSteps.length}`);
    applyAnnotationSteps(finalBuffer, targetWidth, targetHeight, rect.annotationSteps, targetScale);
  } else {
    // 兼容旧数据结构
    if (rect.mosaicRegions && rect.mosaicRegions.length > 0) {
