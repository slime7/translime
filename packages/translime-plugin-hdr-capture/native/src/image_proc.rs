...
pub fn tone_map(
    ...
    for pixel in img.pixels_mut() {
        pixel[0] =
            (apply_tone_curve(pixel[0] as f32 / 255.0, exposure) * 255.0).clamp(0.0, 255.0) as u8;
        pixel[1] =
            (apply_tone_curve(pixel[1] as f32 / 255.0, exposure) * 255.0).clamp(0.0, 255.0) as u8;
        pixel[2] =
            (apply_tone_curve(pixel[2] as f32 / 255.0, exposure) * 255.0).clamp(0.0, 255.0) as u8;
    }
...
fn f16_to_linear(val: u16) -> f32 {
    ...
    let f = if exp == 0 {
        if frac == 0 {
            ...
            (frac as f32) * 2.0f32.powi(-24)
        }
    } else if exp == 0x1F {
        if frac == 0 {
            if sign == 0 { 100.0 } else { 0.0 }
        } else {
            1.0
        }
    } else {
...
pub fn encode_hdr_to_exr(
    ...
    if raw_buffer.len() != expected_size {
        return Err("EXR 编码失败：缓冲区大小不匹配".to_string().into());
    }
    ...
    let layer = Layer::new(
        ...
        SpecificChannels::rgb(|pos: Vec2<usize>| pixels[pos.y() * width + pos.x()]),
    );

    Image::from_layer(layer)
        .write()
        .to_buffered(&mut Cursor::new(&mut out))?;
    Ok(out)
}
